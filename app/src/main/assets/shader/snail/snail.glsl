...
out vec4 fragColor;
//#iChannel0 "assets://shader/snail/ichannel0.png"
uniform sampler2D iChannel0;
//#iChannel0 "assets://shader/snail/ichannel1.png"
uniform sampler2D iChannel1;
//#iChannel0 "assets://shader/snail/ichannel2.png"
...
float mapShell(in vec3 p, out vec4 matInfo)
{
    ...
    float di = textureLod(iChannel1, vec2(t + r, 0.5), 0.).x;
    d += 0.002 * di;
...
vec3 shadeOpaque(in vec3 ro, in vec3 rd, in float t, in float m, in vec4 matInfo)
{
    ...
    if (m < 1.5) // snail body
    {
        ...
        f = 1.0 - pow(f, 8.0);
        f = 1.0 - (1.0 - f) * (1.0 - texture(iChannel1, 0.3 * pos.xy).x);
    ...
    else if (m < 3.5) // plant
    {
        ...
        vec3 te = texture(iChannel1, pos.xy * 0.2).xyz;
        mateS *= 0.5 + 1.5 * te;
    ...
    else //if( m<4.5 ) // leaf
    {
        ...
        vec3 te = texture(iChannel1, p.xz * 0.35).xyz;
        ...
        nor.xz += v * 0.15 * (-1.0 + 2.0 * texture(iChannel2, 1.0 * p.xz).xy);
        nor = normalize(nor);
...
void main()
{
    ...
    for (int n = ZERO; n < AA; n++)
    {
        ...
        float an = 1.87 - 0.04 * (1.0 - cos(0.5 * iTime));
        vec2 q = (gl_FragCoord.xy + rr) / iResolution.xy;
